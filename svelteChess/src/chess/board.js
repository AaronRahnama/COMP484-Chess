...
function run_display() {
  ...
  const socket = io('127.0.0.1:3000', { reconnection: true });
  ...
  //   const urlQuery = new URLSearchParams(window.location.search || window.location.hash);
  const { game_id: g_id } = Http.parseUrlParams();
  const uInfo = Http.getValue('userInfo') || {};
  //https://developer.mozilla.org/en-US/docs/Web/API/URLSearchParams
  let game_id = g_id || uInfo.game_id || getCookie('game_id'); // '';
  let user_id = uInfo.user_id || getCookie('user_id');
  ...
  config.orientation = uInfo.orientation || getCookie('orientation');
  ...
  socket.on('cookie', (data) => {
    ...
    const userInfo = { user_id: data.UID, orientation: data.color, game_id };
    Http.setValue('userInfo', userInfo);
